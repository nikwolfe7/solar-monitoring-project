...
static unsigned char batteryVoltage[] = { 
	0x01,	// Tri-Star device address
	0x03, 	// Function: Read Holding Registers
	0x00,   // Starting Register High
	0x10,   // Starting Register Low
	0x00,   // No. of Registers High
	0x01   	// No. of Registers Low
	};
	
unsigned int battVoltageCmdLen = 6;
...
void TransferProtocol::getBatteryVoltage()
{
	int i;
	unsigned short CRC;
	
	/* Get CRC value */
	CRC = generateCRC_16( batteryVoltage, battVoltageCmdLen );
	
	/* Populate message array */
	unsigned char messageArray[battVoltageCmdLen + 2]; 
	for( i = 0; i < battVoltageCmdLen; ++i )
	{
		messageArray[i] = batteryVoltage[i];
	}
	...
	/* Output data frame */
	for( i = 0; i < (battVoltageCmdLen + 2); ++i )
	{
		Serial.print( messageArray[i], HEX );
	}
	/*
	0x01,0x03,0x00,0x08,0x00,0x01,0x05,0xC8--- expected output
	*/
}
